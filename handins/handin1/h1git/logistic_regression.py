...
class LogisticRegressionClassifier():
    ...
    def cost_grad(self, X, y, w):
        ...
        l1 = logistic(y * (X @ w))
        cost = -np.log(l1.prod())/n


        l2 = logistic((-y) * (X @ w))
        v = -y[:, np.newaxis] * X * l2[:, np.newaxis]

        grad = (np.sum(v, axis=0))/n
        ### END CODE
    ...
    def fit(self, X, y, w=None, lr=0.1, batch_size=16, epochs=10):
        ...
        history = []        
        ### YOUR CODE HERE
        X, y, w, lr, batch_size, epochs

        n, _ = X.shape
        St = np.hstack([X, y[:, np.newaxis]])
        i = 0
        while i < epochs:
            Xy = np.random.permutation(St)
            for j in range(round(n/batch_size)):
                X1 = Xy[batch_size*j:batch_size*(j+1), 0:-1]
                y1 = Xy[batch_size*j:batch_size*(j+1), -1]
                _, g = self.cost_grad(X1, y1, w)
                w -= lr*g
            lr *= 99/100

            cost, _ = self.cost_grad(X, y, w) # Wny this??
            history.append(cost)
            i += 1
        ### END CODE
        self.w = w
